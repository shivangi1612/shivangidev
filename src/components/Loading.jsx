...
export const LoadingScreen = ({ onComplete }) => {
  ...
  return (
    ...
    >
      <div className="w-full max-w-md p-6 rounded-lg bg-zinc-900 border border-primary shadow-2xl">
        {/* Terminal Header */}
        <div className="flex items-center gap-2 mb-4">
          <div className="w-3 h-3 rounded-full bg-red-500" />
          <div className="w-3 h-3 rounded-full bg-yellow-400" />
          <div className="w-3 h-3 rounded-full bg-green-500" />
          <p className="mr-auto text-sm text-zinc-400">Portfolio.jsx</p>
        </div>
        ...
        <div className="text-sm space-y-1 leading-relaxed">
          ...
          {!isComplete && (
            <div className="animate-pulse text-accent">$ <span className="animate-blink">|</span></div>
          )}
        </div>
      </div>
    </motion.div>
